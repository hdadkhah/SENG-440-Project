...
/*		  structure(), printOrder() and padding() functions added   */
/*		  by Rhett Dobson & Hamid Dadkhah	                    */
/*                                                                          */
/****************************************************************************/
...
int germanLetterFrequencies [28] = {651, 189, 306, 508, 1740, 166, 301, 476, 755, 27, 121, 344, 253, 978, 251, 79, 2, 700, 727, 615, 435, 067, 189, 3, 4, 113, 31, 1800};
char germanLetters[28] 	       = {'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'q', 'r', 's', 't', 'u', 'v', 'w', 'x', 'y', 'z', 'L', 'X'};
...
void buildHuffmanTree(Node **tree){
    ...
    for (i=0;i<28;i++){
        ...
        array[i]->value = germanLetterFrequencies[i];
        array[i]->letter = i;
	array[i]->alphabetLetter = germanLetters[i];
        array[i]->left = NULL;
        array[i]->right = NULL;
    }
...
void structure ( struct node *root , int depth)
...
if(root == NULL)
    ...
    printf("%d(%d) %c \n",root->value, depth, root->alphabetLetter);
...
void printOrder( struct node *treePtr, int *row)
{
	if (treePtr != NULL)
	{
		(*row)++;
		...
		if (treePtr->alphabetLetter <1)	
			printf("%c\n", '0');
		else
			printf("%c\n", treePtr->alphabetLetter);
